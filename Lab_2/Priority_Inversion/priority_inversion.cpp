...
int main() {
  ...
  for(int i = 0;i < taskNum;++i) {
    std::cout << "Task properties is..." << std::endl;
    std::cout << "Period=" << taskProperties[i].period << ", Worst case execution time=" << taskProperties[i].execTime << ", Priority=" << taskProperties[i].priority << std::endl << std::endl;
    fflush(stdout);
